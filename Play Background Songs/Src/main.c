...
#define WMW_CMD "wmw"
#define DMP_CMD "dm"
#define SONG_CMD "playSong"
#define BACK_SONG_CMD "playBackgroundSong"
...
int main(void){
	...
	while (1) {
		...
		else if (strcmp(instruction, BACK_SONG_CMD) == 0) {
					//Check the song that is user input, and play it or print error message
					if (strcmp(paramOne, HOTCROSSBUNS) == 0) {
						playSongInBackground(hotCrossBuns);
					}
					else if (strcmp(paramOne, HAPPYBIRTHDAY) == 0) {
						playSongInBackground(happyBirthday);
					}
					else {
						printf("Invalid song. Song options:\n\t"
								" - %s\n\t"
